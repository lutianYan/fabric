...
*/
//TxSimulator和NewQueryExecutor。两者均直接使用helper.go中的queryHelper实现了自身的查询方面的功能
...
func (h *queryHelper) getState(ns string, key string) ([]byte, error) {
	...
	//根据名字空间和key，从stateDB中获取一个状态，返回并写入读集（返回的是值，写入读集的是值的版本）
	//这里的名字空间是chaincodeID，也就是对应一个chaincode的每一个交易使用一个读写集
...
//每获取一个迭代器，都会把迭代器记录在queryHelper的itrs中
//TxSimulator包含QueryExecutor，而提供迭代器也是QueryExecutor对TxSimulator在查询功能上的主要的拓展之一
//依据一个命名空间爱，开始的key和结束的key，从dateDB中获取一个指定范围的交易查询迭代器
