...
//Bundle是一组资源，它们始终具有一致的通道配置视图。 特别是，对于给定的bundle引用，配置序列，策略管理器等将始终返回完全相同的值。 Bundle结构是不可变的，并且将始终用新的后备内存完全替换。
type Bundle struct {
	policyManager   policies.Manager //配置管理器
	mspManager      msp.MSPManager  //MSP组件管理器
