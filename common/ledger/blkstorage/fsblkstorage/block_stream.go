...
//用于读取多个文件
//将单个文件抽象成数据流
//可以从单个文件中顺序读取区块信息，他从给定的偏移地址currentOffset开始读取一直读取到文件末尾
...
//从多个文件中顺序读取区块信息，它从给定的文件的偏移开始一直读到endFileNum的最后
type blockStream struct {
	rootDir           string
	currentFileNum    int
	endFileNum        int
	currentFileStream *blockfileStream //定义指向blockfileStream的指针，用于真正的底层的读取操作
}
...
//该接口每次从指定的文件中的指定位置开始，尝试获取一个block的数据包
